...
## 產出收尾與長輸出守則(2026-07-16 依 /insights 新增;附加條款,不取代上述任何規則)

> 本節不改變任何既有流程:獨立性政策、「不做網頁出版」、零省略 95-105% 字數帶、
> W1-W5 lint、Step 1-4 規則全部照舊,以下只是收尾與輸出方式的補強。

1. **長逐字稿/電子報分段寫檔**:轉錄、校稿、電子報草稿等長內容一律直接分段寫入
   檔案,不要一次性輸出到對話(歷史摩擦:多場 session 因輸出 token 上限報錯,
   逐字稿毀損)。拆分只影響輸出方式,零省略照常適用。
2. **宣告完成前跑 lint 收尾**:動了招生/材料/場次檔案,對應的
   `workshop_lint.py --rule …` 必須全綠才回報完成;涉及數量的內容(講者數、
   場次時長、報名人數、貢獻者數)要與權威來源核對一致。
3. **repo 收尾**:本專案有 GitHub remote(含 workshop2 的同源 clone)。
   要求「存檔/同步」時,commit 之後必 push,並提醒另一份 clone 需要 pull;
   兩份 clone 的 CLAUDE.md 必須保持一致。
4. **事實查核**:電子報/招生文案中的人數、網址、日期,發出前逐一驗證
   (URL 用實際請求確認可達,人數用來源檔案數一遍)。

## 變更紀錄

- **2026-05-26 v0**:WorkShop 專案骨架初版
- **2026-07-16**:新增「產出收尾與長輸出守則」(依 /insights 使用報告;純附加,無既有規則變動)
