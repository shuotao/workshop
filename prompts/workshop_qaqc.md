...
| 階段 | SLA |
|---|---|
| 錄影檔 → cleaned.md(Step 1-2 完成) | **48 小時內** |
| cleaned.md → 寄送觀眾(email body 或 .md 附件) | **7 天內** |

### 交付形式

WorkShop **不做網頁出版**。Meta-Loop 交付給觀眾的是**純 markdown**:
- 短篇可直接貼在 email body
- 長篇以 `.md` 附件寄出(附 NBLM 摘要對照檔可選)
- 招生文宣承諾文字應為「一週內收到一份好學生筆記」,避免暗示有網站連結
...
`workshop_lint.py --rule W5 --event <run>` 計算 (cleaned_md_done_at -
recording_finished_at) 與 (sent_to_audience_at - cleaned_md_done_at) 兩個
時間差,逾期即 ✗。
...
[Meta-Loop 後處理]
  ─ 錄影 → Step 1-2 → Step 3-4 → 整理成 markdown
  ─ 觸發: workshop_lint.py --rule W5 --event <run>
        ↓ 全綠才寄 .md 給觀眾
```
...
任何 W 失敗 → 上游 R(轉錄/校稿)也應該重審,因為錯誤可能源自更深層。
...
- **2026-05-26 v3**:WorkShop 確認不做網頁出版,刪除 W6(出版規則承襲);W5
  改寫為 markdown email 交付 SLA(原 publish-上線/notification 三段壓縮成兩段)
